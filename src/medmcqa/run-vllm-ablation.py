...
def main(
    ...
    if not disable_thinking:
        save_file = f"/mnt/blob_output/v-dachengwen/LLM-Stability-v2/outputs/medmcqa/{_model_name}_temp{str(temperature)}_n{str(n)}_step1.jsonl"
    else:
        save_file = f"/mnt/blob_output/v-dachengwen/LLM-Stability-v2/outputs/medmcqa/{_model_name}_temp{str(temperature)}_n{str(n)}_dt_step1.jsonl"
