...
- Do not alter, paraphrase, or simplify any medical terminology (e.g., disease names, drug names, anatomical structures, lab values, or other clinical terms).
...
if __name__ == "__main__":
    ...
    save_file = f"datasets/medmcqa/medmcqa_non_mcq.jsonl"
    idx_done = []
    ...
    all_idx = set()
    line_count = 0
    with open(f"datasets/medmcqa/medmcqa_paraphrase.jsonl", "r") as f:
        for line in f:
            line_count += 1
            if line_count <= 250:
                continue
            ...
            idx = str(item["idx"])
            all_idx.add(idx)
            all_paraphrases = item["paraphrases"]
            # if len(all_paraphrases) < 30:
            #     continue
            # if len(all_idx) >= 400:
            #     break
