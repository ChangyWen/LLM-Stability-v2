...
def main(
    ...
    test_data_file = "/mnt/blob_output/v-dachengwen/LLM-Stability-v2/datasets/moralchoice/moralchoice_en.jsonl"
    os.makedirs("/mnt/blob_output/v-dachengwen/LLM-Stability-v2/outputs/moralchoice/", exist_ok=True)
    _model_name = model_name.split("/")[-1]

    if not disable_thinking:
        save_file = f"/mnt/blob_output/v-dachengwen/LLM-Stability-v2/outputs/moralchoice/{_model_name}_temp{str(temperature)}_n{str(n)}.jsonl"
    else:
        save_file = f"/mnt/blob_output/v-dachengwen/LLM-Stability-v2/outputs/moralchoice/{_model_name}_temp{str(temperature)}_n{str(n)}_dt.jsonl"
